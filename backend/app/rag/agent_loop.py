...
# RE-EXPORTED, not redefined. These moved to `app/rag/textguard.py` so the ADK
# runtime can import the same objects rather than a second copy of them; the names
# stay importable from here because `scripts/refusal_check.py` (cases 27-33) and
# `scripts/agentic_check.py` both reach for them at this path, and a move that
# breaks a harness import is a move that gets reverted for the wrong reason.
from app.rag.textguard import (  # noqa: F401  -- re-export for compatibility
    _LEAKED_TOOL_MARKUP,
    _emit_until_markup,
...
# The leaked-tool-markup guard MOVED to `app/rag/textguard.py` on 2026-08-28,
# unchanged, because `app/adk/` needs the identical guard and CLAUDE.md's rule is
# that a contract stated twice drifts. The three names are re-exported at the top
# of this module, so `scripts/refusal_check.py` cases 27-33 and every existing
# import keep resolving against `app.rag.agent_loop`.
