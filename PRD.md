...
| ~~23~~ | ~~**Tool use is unmeasured**~~ | ✅ **done 2026-08-23 (change set 16), and the warning in the original wording is what shaped it.** Trajectory evaluation ships as TWO things of deliberately different kinds, because "inventing a faithfulness-shaped score for tool choice" was the failure to avoid. The **judged** half is `AgentGoalAccuracyWithReference`, which Ragas already ships and which was calibrated against a known-good and a known-bad trajectory before it was trusted — `agent_metrics_check.py --live` case 22 asserts the two verdicts DIFFER, because a metric returning a constant passes either case alone. The **counted** half invents nothing: `expected_tool_use` on a golden question is a proposition (`search` / `none` / `python` / NULL) and the verdict is arithmetic over `trace_events`. **Ragas' own two tool-call metrics were measured and REJECTED** — both compare arguments byte-exactly against a query this system rewrites every turn at temperature 1.0, scoring a correct agent 0.0; see item 49 |
...
| **49** | **Ragas' two tool-call metrics cannot score this agent** | **Closed by measurement, not deferred** — recording it so the idea does not return. `ToolCallAccuracy` and `ToolCallF1` both compare tool ARGUMENTS byte-exactly, and `REWRITE_EVERY_TURN=true` rewrites `search_corpus(query=...)` on every turn at temperature 1.0, so the one meaningful argument is designed never to repeat. Measured: **0.0** for a differently worded query, **0.0** for an empty reference arg dict (`_get_arg_score` reads `if not refs: return 0.0`, so the obvious escape hatch fails exactly where it would be used), and **0.0** for two calls against one reference — which is the 1.50–2.00 calls/step this model normally emits. `ToolCallF1` has no comparator field at all. Reopening this needs either a ragas release that changes `_get_arg_score` (`agent_metrics_check.py` case 6 goes RED and tells you) or a decision to stop rewriting eval queries |
| **50** | **`AgentGoalAccuracyWithoutReference` and `TopicAdherenceScore` are unused** | Open, deliberately. The first infers the goal AND the outcome from the same conversation, so it grades against something it wrote itself — the defect three vendors were introduced to remove. The second needs `reference_topics` nobody has authored and scores MULTI-TURN conversations, while an eval run creates one conversation per question. Worth revisiting only if eval ever goes multi-turn: its recall term is an LLM-graded refusal measure independent of the marker list that has been wrong five times |
...
(open item 23, Stage 4), not a faithfulness-shaped one. **Item 23 shipped 2026-08-23**, so the instrument now
exists: `AgentGoalAccuracyWithReference` scores whether the turn achieved what was
asked without scoring the individual sentences, which is exactly the property this
defect needs. Wiring the self-check's own firing into a golden set is still unbuilt.
